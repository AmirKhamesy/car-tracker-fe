...
export default function Car(props: any) {
    ...
    const carTitleElement = <h2 className="title text-start">{props.car.year + " " + props.car.make + " " + props.car.model}</h2>;
    const priceElement = <h2 className="title text-start">${props.car.price}</h2>;
    const editableMake = <input
        type="text"
    ...
    const displayCarDetails = <div>
        {carTitleElement} <br />
        {priceElement}
    </div>

    const editCarDetails = <div>
        {editableMake}
        {editableModel}
        {editableYear}
        {editablePrice}
